...
template <typename S>
concept State =
    requires(S s, typename S::BoardType board, typename S::BBType bitboard,
             std::vector<typename S::BBType> compact,
             const std::string &state_str, typename S::Player player) {
        ...
        { s.to_string() } -> std::same_as<std::string>;
        { s.from_string(state_str) } -> std::same_as<void>;
    };
