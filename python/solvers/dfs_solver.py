...
class DepthFirstSearch:
    ...
    def traverse(self, game: GameProtocol, node: Node) -> float:
        ...
        policy = np.zeros(9)
        policy[np.argmax(action_values)] += 1.0
...
if __name__ == "__main__":
    ...
    file_path = "./python/ttt_oh_dataset.h5"
    ...
    with h5py.File(file_path, "w") as f:
        ...
        training.create_dataset("states", data=states[indices[:3478]])
        training.create_dataset("masks", data=masks[indices[:3478]])
        training.create_dataset("values", data=values[indices[:3478]])
        training.create_dataset("policies", data=policies[indices[:3478]])
        testing.create_dataset("states", data=states[indices[3478:]])
        testing.create_dataset("masks", data=masks[indices[3478:]])
        testing.create_dataset("values", data=values[indices[3478:]])
        testing.create_dataset("policies", data=policies[indices[3478:]])
    # with h5py.File(file_path, "w") as f:
    #     training = f.create_group("training")
    #     testing = f.create_group("testing")
