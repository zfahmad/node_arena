...
class ResidualBlock(nnx.Module):
    def __init__(self, rngs: nnx.Rngs):
        ...
        self.ln2 = nnx.LayerNorm(64, rngs=rngs)
        self.conv3 = nnx.Conv(64, 32, kernel_size=(1, 1), rngs=rngs, padding="SAME")
        self.ln3 = nnx.LayerNorm(32, rngs=rngs)

    def __call__(self, input_):
        x = self.conv1(input_)
        x = nnx.relu(self.ln1(x))
        x = self.conv2(x)
        x = nnx.relu(self.ln2(x))
        x = self.conv3(x)
        x = nnx.relu(self.ln3(x))
...
class ValueHead(nnx.Module):
    def __init__(self, in_features: int, rngs: nnx.Rngs):
        ...
        self.ln1 = nnx.LayerNorm(128, rngs=rngs)
...
class PolicyHead(nnx.Module):
    def __init__(self, in_features: int, policy_len: int, rngs: nnx.Rngs):
        self.conv = nnx.Conv(8, 8, (3, 3), rngs=rngs, padding="SAME")
        self.ln1 = nnx.LayerNorm(8, rngs=rngs)
        self.linear1 = nnx.Linear(in_features, 128, rngs=rngs)
    ...
    def __call__(self, input_):
        x = nnx.relu(self.ln1(self.conv(input_)))
        x = x.reshape(x.shape[0], -1)
        x = nnx.relu(self.ln2(self.linear1(x)))
        x = nnx.relu(self.ln3(self.linear2(x)))
        p = self.linear3(x)
        return p
...
class ResNet(nnx.Module):
    ...
    def __call__(self, input_):
        x = nnx.relu(self.ln1(self.conv1(input_)))
        x = nnx.relu(self.ln2(self.conv2(x)))

        for res_block in self.blocks:
            x = res_block(x)

        x = self.ln3(self.ch_red(x))
